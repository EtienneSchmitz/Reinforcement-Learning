...
cost_internal_goal = -0.7  # -0.5
cost_internal_w_goal = -0.07  # -0.05
cost_internal_self_thinking = -1  # -1
cost_internal_self_thinking_cc = -0.5
bad_goal = -6.0
good_goal = 9.5
illegal_action = -1
legal_move = -0.05   # 0.05
...
if __name__ == '__main__':
    ...
    for n_run in range(40):
        ...
        # plt.show()
        final_perf = np.average(correct_responses, axis=0)[-1]
        if final_perf<0.7 and final_perf>0.3:
            average_half +=1
        elif final_perf>0.7:
            average_one +=1
        else:
    ...
    print("Number of perf going toward 1 : ",average_one)
    print("Number of perf going toward 0.5 : ",average_half)
    print("Absolute failures : ", absolute_failure)